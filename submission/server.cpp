...
void Server::doSomethingWithReceivedData(string& message) {
    ...
    if(paxosMsg.getType() == "GETSIZE"){
        ...
        while(sended_count >= 0){
            maxSlot = max(maxSlot, paxosMsg.getSlot());
            sended_count--;
        }
        ...
        if(maxSlot > 0){
            ...
            if((log.getEvents()).size() == maxSlot){
                curSlot = maxSlot;
            }else{
                curSlot = maxPrepare.size();
                fillUpLogSynod();
            }
        }
...
void Server::fillUpLogSynod(){
    ...
    while(slot < maxSlot){
        cout << "slot before sending prepare is " << slot << endl;
        prepare(slot);
        slot++;
    }
...
void Server::synod(){
    ...
    if(curSlot!=0 && accVal[curSlot-1].getUserId()==userId) {
        ...
        cout << "slot before synod prepare is " << slot << endl;
